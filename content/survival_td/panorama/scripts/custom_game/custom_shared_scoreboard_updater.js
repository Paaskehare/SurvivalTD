...
function _ScoreboardUpdater_UpdatePlayerPanel( scoreboardConfig, playersContainer, playerId, localPlayerTeamId )
{
	...
	if ( playerInfo )
	{
		...
		_ScoreboardUpdater_SetTextSafe( playerPanel, "Tier", playerInfo.player_level );
...
//		$.Msg( "UpdateTeamPanel.Create: ", teamPanelName, " = ", scoreboardConfig.teamXmlName );
	...
	_ScoreboardUpdater_SetTextSafe( teamPanel, "Lives", g_GameState.Lives[teamId] );

	var DifficultyLevel = g_GameState.Difficulty[teamId];
	var DifficultyLevelString = (DifficultyLevel === 2 ? 'hard' : (DifficultyLevel === 1 ? 'normal' : 'easy'));
	
	_ScoreboardUpdater_SetTextSafe( teamPanel, "Difficulty", $.Localize('custom_game_difficulty_' + DifficultyLevelString) ); 

	var TeamStateInfo = g_GameState.TeamState[teamId];
	_ScoreboardUpdater_SetTextSafe( teamPanel, "Round", TeamStateInfo.Round);
